...
void Test::framebufferResizeCallback(GLFWwindow* window, int width, int height) {
...
bool Test::CheckExtensionSupport(VkPhysicalDevice device){
...
void Test::CreateSwapChain(){
    SwapChainSupportDetails details = GetSwapChainSupportDetails(_device);
    ...
    swapchainCreateInfo.presentMode = presentMode;
         VkResult result = vkCreateSwapchainKHR(_logicDevice,&swapchainCreateInfo,nullptr,&_swapChain);
     if(result != VK_SUCCESS){
...
void Test::GetSwapChainImages(std::vector<VkImage>& images){
    uint32_t imageCount;
    vkGetSwapchainImagesKHR(_logicDevice,_swapChain,&imageCount,nullptr);
    if(imageCount!=0){
        images.resize(imageCount);
        vkGetSwapchainImagesKHR(_logicDevice,_swapChain,&imageCount,images.data());
    }
...
void Test::CreateGraphicsPipeline(){
    ...
    if (vkCreatePipelineLayout(_logicDevice, &pipelineLayoutInfo, nullptr, &_pipelineLayout) != VK_SUCCESS) {
    ...
    if (vkCreateGraphicsPipelines(_logicDevice, VK_NULL_HANDLE, 1, &GPCreateInfo, nullptr, &_pipeline) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create graphics pipeline!");
    }

    vkDestroyShaderModule(_logicDevice,vert,nullptr);
    vkDestroyShaderModule(_logicDevice,frag,nullptr);
...
void Test::CreateDescriptorSetLayout(){
    ...
    if (vkCreateDescriptorSetLayout(_logicDevice, &layoutInfo, nullptr, &_descriptorSetLayout) != VK_SUCCESS) {
        throw std::runtime_error("Failed to create descriptor set layout!");
    }
    std::cout << "[SUCCESS] Descriptor set layout created" << std::endl;
}

void Test::DestroyDebugMessenger() {
    if (enabledValidationLayer) {
...
void Test::CreateLogicDevice(){
    ...
    VkResult result = vkCreateDevice(_device, &logicInfo, nullptr, &_logicDevice);
    if(result != VK_SUCCESS){
...
void Test::CreateBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer& buffer, VkDeviceMemory& bufferMemory) {
    ...
    if (vkCreateBuffer(_logicDevice, &bufferInfo, nullptr, &buffer) != VK_SUCCESS) {
        throw std::runtime_error("failed to create vertex buffer!");
    }
    VkMemoryRequirements memRequirements;
    vkGetBufferMemoryRequirements(_logicDevice, buffer, &memRequirements);
    VkMemoryAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex = FindMemoryType(memRequirements.memoryTypeBits, properties);
    if (vkAllocateMemory(_logicDevice, &allocInfo, nullptr, &bufferMemory) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate vertex buffer memory!");
    }
    vkBindBufferMemory(_logicDevice, buffer, bufferMemory, 0);
}

void Test::CopyBuffer(VkBuffer src,VkBuffer dst,VkDeviceSize size){
...
void Test::CreateUniformBuffers(){
    ...
    for(int i=0;i<MAX_FRAMES_IN_FLIGHT;i++){
        vkMapMemory(_logicDevice, _uniformBuffersMemory[i], 0, bufferSize, 0, &_uniformBuffersMapped[i]);
    }
...
void Test::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout){
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
void Test::TransitionImageLayout(VkImage image, VkFormat format, uint32_t mipLevels, VkImageLayout oldLayout, VkImageLayout newLayout){
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
    VkImageMemoryBarrier barrier{};
    barrier.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER;
    barrier.oldLayout = oldLayout;
...
void Test::CopyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height){
    VkCommandBuffer commandBuffer = BeginSingleTimeCommands();
...
void Test::CreateDescriptorPool(){
    ...
    if (vkCreateDescriptorPool(_logicDevice, &poolInfo, nullptr, &_descriptorPool) != VK_SUCCESS) {
        throw std::runtime_error("failed to create descriptor pool!");
    }
    std::cout<<"[SUCCESS] Descriptor pool created"<<std::endl;
}

void Test::CreateDescriptorSets(){
    ...
    if (vkAllocateDescriptorSets(_logicDevice, &allocInfo, _descriptorSets.data()) != VK_SUCCESS) {
    ...
    for(int i=0;i<MAX_FRAMES_IN_FLIGHT;i++){
        ...
        vkUpdateDescriptorSets(_logicDevice, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);  
    }
    std::cout<<"[SUCCESS] Descriptor sets created"<<std::endl;
}

void Test::CreateTextureImage(){
    ...
    void* data;
    vkMapMemory(_logicDevice, stagingBufferMemory, 0, imageSize, 0, &data);
...
void Test::CreateTextureImageView(){
...
void Test::CreateTextureSampler(){
    ...
    if (vkCreateSampler(_logicDevice, &samplerInfo, nullptr, &_textureSampler) != VK_SUCCESS) {
        throw std::runtime_error("failed to create texture sampler!");
    }
    std::cout<<"[SUCCESS] Texture sampler created"<<std::endl;
}

void Test::CreateImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory){
    ...
    if (vkCreateImage(_logicDevice, &imageInfo, nullptr, &image) != VK_SUCCESS) {
        throw std::runtime_error("failed to create image!");
    }

    VkMemoryRequirements memRequirements;
    vkGetImageMemoryRequirements(_logicDevice, image, &memRequirements);
    ...
    if (vkAllocateMemory(_logicDevice, &allocInfo, nullptr, &imageMemory) != VK_SUCCESS) {
        throw std::runtime_error("failed to allocate image memory!");
    }

    vkBindImageMemory(_logicDevice, image, imageMemory, 0);
}

void Test::CreateImage(uint32_t width, uint32_t height,uint32_t mipLevels, VkSampleCountFlagBits numSamples, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image, VkDeviceMemory& imageMemory){
    VkImageCreateInfo imageInfo{};
    imageInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
    imageInfo.imageType = VK_IMAGE_TYPE_2D;
    imageInfo.extent.width = width;
...
VkImageView Test::CreateImageView(VkImage image, VkFormat format,uint32_t mipLevels, VkImageAspectFlags aspectFlags){
    ...
    if (vkCreateImageView(_logicDevice, &viewInfo, nullptr, &imageView) != VK_SUCCESS) {
        throw std::runtime_error("failed to create image view!");
    }
    return imageView;
}

VkImageView Test::CreateImageView(VkImage image, VkFormat format,VkImageAspectFlags aspectFlags){
    VkImageViewCreateInfo viewInfo{};
    viewInfo.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO;
    viewInfo.image = image;
    viewInfo.viewType = VK_IMAGE_VIEW_TYPE_2D;
...
void Test::GenerateMipmaps(VkImage image, VkFormat imageFormat, int32_t texWidth, int32_t texHeight, uint32_t mipLevels){
    if (mipLevels == 1) {
        return;
    }
    VkFormatProperties formatProperties;
    vkGetPhysicalDeviceFormatProperties(_device, imageFormat, &formatProperties);
...
void Test::CreateDepthResources(){
...
void Test::CreateColorResources(){
...
VkSampleCountFlagBits Test::GetMaxUsableSampleCount(){
...
bool Test::IsDeviceSuitable(VkPhysicalDevice device){
...
QueueFamily Test::FindQueueFamilyIndex(VkPhysicalDevice device){
...
void Test::CleanupVulkan(){
    ...
    vkFreeMemory(_logicDevice, _textureImageMemory, nullptr);
    vkDestroyBuffer(_logicDevice, _indexBuffer, nullptr);
    vkFreeMemory(_logicDevice, _indexBufferMemory, nullptr);
    vkDestroyBuffer(_logicDevice, _vertexBuffer, nullptr);
    vkFreeMemory(_logicDevice, _vertexBufferMemory, nullptr);
    vkDestroyDescriptorPool(_logicDevice, _descriptorPool, nullptr);
    for(int i=0;i<MAX_FRAMES_IN_FLIGHT;i++){
    vkDestroySemaphore(_logicDevice, _imageAvailableSemaphores[i], nullptr);
    vkDestroySemaphore(_logicDevice, _renderFinishedSemaphores[i], nullptr);
    vkDestroyFence(_logicDevice, _inFlightFences[i], nullptr);
    }
    vkDestroyCommandPool(_logicDevice, _commandPool, nullptr);
    vkDeviceWaitIdle(_logicDevice);
    vkDestroyPipeline(_logicDevice,_pipeline,nullptr);
    vkDestroyPipelineLayout(_logicDevice,_pipelineLayout,nullptr);
    vkDestroyDescriptorSetLayout(_logicDevice,_descriptorSetLayout,nullptr);
...
void Test::CleanupSwapChain(){
    vkDestroyImageView(_logicDevice, _colorImageView, nullptr);
    vkDestroyImage(_logicDevice, _colorImage, nullptr);
    vkFreeMemory(_logicDevice, _colorImageMemory, nullptr);
    vkDestroyImageView(_logicDevice, _depthImageView, nullptr);
    vkDestroyImage(_logicDevice, _depthImage, nullptr);
    vkFreeMemory(_logicDevice, _depthImageMemory, nullptr);
    for(auto& iv : _imageViews){
        vkDestroyImageView(_logicDevice,iv,nullptr);
    }
    vkDestroySwapchainKHR(_logicDevice, _swapChain , nullptr);
}

void Test::RecreateSwapChain(){
...
SwapChainSupportDetails Test::GetSwapChainSupportDetails(VkPhysicalDevice device){
...
VkSurfaceCapabilitiesKHR Test::GetSwapChainCapabilities(VkPhysicalDevice device){
    VkSurfaceCapabilitiesKHR capabilities;
    vkGetPhysicalDeviceSurfaceCapabilitiesKHR(device, _surface, &capabilities);
    return capabilities;
}

std::vector<VkSurfaceFormatKHR> Test::GetSwapChainFormat(VkPhysicalDevice device){
...
std::vector<VkPresentModeKHR> Test::GetSwapChainPresentMode(VkPhysicalDevice device){
...
VkSurfaceFormatKHR Test::ChooseSurfaceFMT(const SwapChainSupportDetails& swapChain){
...
VkPresentModeKHR Test::ChoosePresentMode(const SwapChainSupportDetails& swapchain){
...
VkExtent2D Test::ChooseExtent2D(const SwapChainSupportDetails& swapchain){
...
VkShaderModule Test::CreateShaderModule(const std::vector<char>& code){
    ...
    VkResult result;
    result = vkCreateShaderModule(_logicDevice,&SMCreateInfo,nullptr,&shaderModule);
    if (result != VK_SUCCESS) {
        switch (result) {
            case VK_ERROR_OUT_OF_HOST_MEMORY:
...
VkCommandBuffer Test::BeginSingleTimeCommands() {
    VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    allocInfo.commandPool = _commandPool;
    ...
    vkAllocateCommandBuffers(_logicDevice, &allocInfo, &commandBuffer);
...
void Test::EndSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void Test::BeginCommandBuffer(VkCommandBuffer commandBuffer, VkCommandBufferUsageFlags flags){
    VkCommandBufferBeginInfo beginInfo{};
    beginInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO;
...
void Test::EndCommandBuffer(VkCommandBuffer commandBuffer){
...
uint32_t Test::FindMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties){
...
VkFormat Test::FindSupportedFormat(const std::vector<VkFormat>& candidates, VkImageTiling tiling, VkFormatFeatureFlags features){
...
VkFormat Test::FindDepthFormat(){
    return FindSupportedFormat({VK_FORMAT_D32_SFLOAT, VK_FORMAT_D32_SFLOAT_S8_UINT, VK_FORMAT_D24_UNORM_S8_UINT}, VK_IMAGE_TILING_OPTIMAL, VK_FORMAT_FEATURE_DEPTH_STENCIL_ATTACHMENT_BIT);
}

void Test::DrawFrame(){
    vkWaitForFences(_logicDevice, 1, &_inFlightFences[currentFrame], VK_TRUE, std::numeric_limits<uint64_t>::max());
